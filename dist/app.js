...
if (document.modelContext?.registerTool) {
  ...
  try {
    Promise.resolve(document.modelContext.registerTool({
      ...
      inputSchema: { type: 'object', properties: { services: { type: 'array', items: { type: 'string', enum: allowedServices }, minItems: 1, uniqueItems: true }, name: { type: 'string', minLength: 1, maxLength: 100 }, zip: { type: 'string', pattern: '^[0-9]{5}$' }, phone: { type: 'string', maxLength: 30 }, details: { type: 'string', maxLength: 1800 } }, required: ['services', 'name', 'zip'], additionalProperties: false },
      annotations: { readOnlyHint: false, untrustedContentHint: false },
      execute(input) {
        if (!input || typeof input !== 'object' || Object.keys(input).some(key => !['services', 'name', 'zip', 'phone', 'details'].includes(key)) || !Array.isArray(input.services) || !input.services.length || input.services.some(service => !allowedServices.includes(service)) || new Set(input.services).size !== input.services.length || typeof input.name !== 'string' || !input.name.trim() || input.name.length > 100 || typeof input.zip !== 'string' || !/^[0-9]{5}$/.test(input.zip) || (input.phone !== undefined && (typeof input.phone !== 'string' || input.phone.length > 30)) || (input.details !== undefined && (typeof input.details !== 'string' || input.details.length > 1800))) throw new Error('Provide a valid name, five-digit ZIP, and supported services.');
        serviceInputs.forEach(checkbox => { checkbox.checked = input.services.includes(checkbox.value); });
        for (const key of ['name', 'zip', 'phone', 'details']) quoteForm.elements.namedItem(key).value = input[key] || '';
        quoteForm.elements.namedItem('name').setCustomValidity(''); syncServices();
